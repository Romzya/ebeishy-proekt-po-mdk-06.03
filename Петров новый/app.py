...
if not SECRET_KEY:
    raise RuntimeError(
        'SECRET_KEY не задан. Добавьте его в файл .env или переменные окружения.'
    )
...
class Order(db.Model):
    ...
    card_masked = db.Column(db.String(30), nullable=False)
...
def is_safe_url(target):
    """Проверяет, что URL ведёт на наш же домен."""
    if not target:
        return False
...
@app.route('/')
def index():
    featured = Toy.query.limit(4).all()
...
def catalog():
    ...
    if search:
        like = f'%{search}%'
        query = query.filter(db.or_(Toy.name.ilike(like), Toy.description.ilike(like)))
    if sort == 'price_asc':
...
def checkout():
    ...
    if request.method == 'POST':
        ...
        if not re.match(r'^(0[1-9]|1[0-2])\/\d{2}$', card_expiry):
            errors.append('Срок действия должен быть в формате ММ/ГГ')
